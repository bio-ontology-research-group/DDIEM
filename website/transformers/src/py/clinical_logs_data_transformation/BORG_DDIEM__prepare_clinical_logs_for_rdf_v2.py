...
import urllib.parse;import urllib.request;import json;
...
def run_BORG_DDIEM__prepare_clinical_logs_for_rdf_v2(
    w
    ,queue
    ,worker_id
):
    try:
        ...
        d.stop();
class BORG_DDIEM__prepare_clinical_logs_for_rdf_v2():
    def __init__(
        self
        ...
        ,_gene_symbol__2__ko_id_tsv_file_name
        ...
        self._gene_symbol__2__ko_id_tsv_file_name=_gene_symbol__2__ko_id_tsv_file_name;
        ...
        try:
            ...
            if(_gene_symbol__2__ko_id_tsv_file_name==None or len(_gene_symbol__2__ko_id_tsv_file_name.strip())<0):
                pass;
                #raise ValueError("_gene_symbol__2__ko_id_tsv_file_name is empty the supplied value is '%s'"%(_gene_symbol__2__ko_id_tsv_file_name));
    ...
    def run(self):
        ...
        #######obtain PubChem CID metadata for distinct pubChem_CID__list 2020-04-16 2030hrs (start)
        #dest_dataset_json_file_name=os.path.join(self._dest_dir_file_name,"%s.clinical_logs_for_rdf_v2.drugbank_drug_names.json"%(os.path.splitext(os.path.basename(self._src_clinical_log_dataset_csv_file_name))[0]));
        self._pubChem_CID_info__dict, self._pubChem_CID__list2=obtain_pubChem_CID_info(
            self._distinct_ids__list__dict["pubChem_CID__list"]
        );
        #######obtain PubChem CID metadata for distinct pubChem_CID__list 2020-04-16 2310hrs (end)
        """
        ...
        #######obtain PubChem SID metadata for distinct pubChem_SID__list 2020-04-17 0855hrs (start)
        #dest_dataset_json_file_name=os.path.join(self._dest_dir_file_name,"%s.clinical_logs_for_rdf_v2.drugbank_drug_names.json"%(os.path.splitext(os.path.basename(self._src_clinical_log_dataset_csv_file_name))[0]));
        self._pubChem_SID_info__dict, self._pubChem_SID__list2=obtain_pubChem_SID_info(
            self._distinct_ids__list__dict["pubChem_SID__list"]
        );
        #######obtain PubChem SID metadata for distinct pubChem_SID__list 2020-04-17 0858hrs (end)
        self._distinct_ids__list__dict["pubChem_SID__list2"]=self._pubChem_SID__list2;
        ...
        self._gene_info__dict, self._gene_id__list=obtain_gene_info(
        ...
        self._gene_id__2__uniprotkb_id__results__dict=obtain_gene_id__2__uniprotkb_id(
            self._distinct_ids__list__dict["gene_id__list"]
        );
        """
        self._gene_id__2__uniprotkb_id__dict, self._gene_symbol__list=package_gene_id__2__uniprotkb_id(
            self._gene_id__2__uniprotkb_id_tsv_file_name
            ,self._distinct_ids__list__dict["gene_id__list"]
        );
        """;
        if("uniprotkb_id__list" in self._gene_id__2__uniprotkb_id__results__dict):
            if(not "uniprotkb_id__list" in self._distinct_ids__list__dict):
                self._distinct_ids__list__dict["uniprotkb_id__list"]=[];
            self._distinct_ids__list__dict["uniprotkb_id__list"].extend(self._gene_id__2__uniprotkb_id__results__dict["uniprotkb_id__list"]);
            self._distinct_ids__list__dict["uniprotkb_id__list"]=list(set(self._distinct_ids__list__dict["uniprotkb_id__list"]));
            self._distinct_ids__list__dict["uniprotkb_id__list"].sort(reverse=False);
        #LOGGER.info("self._distinct_ids__list__dict[\"gene_symbol__list\"] is:'%s'"%(json.dumps(self._distinct_ids__list__dict["gene_symbol__list"],indent=4)));
        #LOGGER.info("self._gene_id__2__uniprotkb_id__dict is:'%s'"%(json.dumps(self._gene_id__2__uniprotkb_id__dict,indent=4)));
        self._ko_id__list=None;
        self._uniprotkb_id__2__ko_id__results__dict=obtain_uniprotkb_id__2__ko_id(
            self._distinct_ids__list__dict["uniprotkb_id__list"]
        );
        if("ko_id__list" in self._uniprotkb_id__2__ko_id__results__dict):
            if(not "ko_id__list" in self._distinct_ids__list__dict):
                self._distinct_ids__list__dict["ko_id__list"]=[];
            self._distinct_ids__list__dict["ko_id__list"].extend(self._uniprotkb_id__2__ko_id__results__dict["ko_id__list"]);
            self._distinct_ids__list__dict["ko_id__list"]=list(set(self._distinct_ids__list__dict["ko_id__list"]));
            self._distinct_ids__list__dict["ko_id__list"].sort(reverse=False);
            self._ko_id__list=self._distinct_ids__list__dict["ko_id__list"];
        
        """
        self._gene_symbol__2__ko_id__dict, self._ko_id__list=obtain_gene_symbol__2__ko_id(
            self._distinct_ids__list__dict["gene_symbol__list"]
        );
        """;
        """        
        self._gene_symbol__2__ko_id__dict, self._ko_id__list=package_gene_symbol__2__ko_id(
            self._gene_symbol__2__ko_id_tsv_file_name
            ,self._distinct_ids__list__dict["gene_symbol__list"]
        );
        """;
        self._distinct_ids__list__dict["ko_id__list"]=self._ko_id__list;
        #LOGGER.info("self._distinct_ids__list__dict[\"gene_symbol__list\"] is:'%s'"%(json.dumps(self._distinct_ids__list__dict["gene_symbol__list"],indent=4)));
        #LOGGER.info("self._gene_symbol__2__ko_id__dict is:'%s'"%(json.dumps(self._gene_symbol__2__ko_id__dict,indent=4)));
        ...
        ####Write the results dictionaries to file (start) 2020-04-22 0953hrs#######################
        dest_dataset_json_file_name=os.path.join(self._dest_dir_file_name,"%s.%s.json"%(os.path.splitext(os.path.basename(self._src_clinical_log_dataset_csv_file_name))[0],"gene_id__2__uniprotkb_id__results__dict"));
        dest_dataset_json_fh=None;
        dest_dataset_json_writer=None;
        dest_dataset_json_fh=open(dest_dataset_json_file_name,"wb");
        json.dump(self._gene_id__2__uniprotkb_id__results__dict, codecs.getwriter("utf-8")(dest_dataset_json_fh), ensure_ascii=False, indent=4);
        dest_dataset_json_fh.flush();
        dest_dataset_json_fh.close();
        dest_dataset_json_fh=None;
        
        dest_dataset_json_file_name=os.path.join(self._dest_dir_file_name,"%s.%s.json"%(os.path.splitext(os.path.basename(self._src_clinical_log_dataset_csv_file_name))[0],"uniprotkb_id__2__ko_id__results__dict"));
        dest_dataset_json_fh=None;
        dest_dataset_json_writer=None;
        dest_dataset_json_fh=open(dest_dataset_json_file_name,"wb");
        json.dump(self._uniprotkb_id__2__ko_id__results__dict, codecs.getwriter("utf-8")(dest_dataset_json_fh), ensure_ascii=False, indent=4);
        dest_dataset_json_fh.flush();
        dest_dataset_json_fh.close();
        dest_dataset_json_fh=None;
        ####Write the results dictionaries to file (end) 2020-04-22 1014hrs#######################
        
            
        
        
        ####Write the results dictionaries to file (end) 2020-04-22 0953hrs#######################
        ...
        dest_dataset_json_file_name=os.path.join(self._dest_dir_file_name,"%s.clinical_logs_for_rdf_v2.ChEBI_drug_names.json"%(os.path.splitext(os.path.basename(self._src_clinical_log_dataset_csv_file_name))[0]));
        dest_dataset_json_fh=None;
        dest_dataset_json_writer=None;
        dest_dataset_json_fh=open(dest_dataset_json_file_name,"wb");
        import codecs;
        ...
        if(1==1):
            self._processing_outcome__dict=cascade_fields_values(
                ...
                ,self._omim_id__dict
                ,self._gene_id__2__uniprotkb_id__results__dict
                ,self._uniprotkb_id__2__ko_id__results__dict
...
def obtain_pubChem_SID_description(
    pubChem_SID
):
    response__dict=None;
    #import urllib.parse;import urllib.request;import json;
    #url = "https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/sid/472181/description/JSON";params = {};
    url = "https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/sid/{0}/description/JSON".format(pubChem_SID);params = {};
    LOGGER.info("url is:'{0}', params are:'{1}'".format(url,json.dumps(params,indent=4,sort_keys=False)));
    ...
    try:
        ...
        #print("response__dict is:'{0}'".format(json.dumps(response__dict,indent=4,sort_keys=False)));
        """
        {
          "InformationList": {
            "Information": [
              {
                "SID": 472181,
                "Title": "Copper dimethyldithiocarbamate"
              }
            ]
          }
        }
        ...
        LOGGER.error("pubChem_SID is:'{0}', An error has been detected. Reason:'{1}'".format(pubChem_SID,error));
        logging.exception("pubChem_SID is:'{0}', An error has been detected. Reason:'{1}'".format(pubChem_SID,error));
        #traceback.print_exc();
        print(
        ...
        pass;#
    return response__dict;
    
    
def obtain_pubChem_SID_info(
    _pubChem_SID__list
):
    pass;
    ...
    for i, _pubChem_SID in enumerate(_pubChem_SID__list):
        record_ordinal_position=i;
        _pubChem_SID_number=_pubChem_SID.split("SID:")[-1];
        response__dict=obtain_pubChem_SID_description(
            _pubChem_SID_number
        );
        time.sleep(0.2);
        LOGGER.info(">>>>>>>>>>>>>>>>>>>>>>>>>>>_pubChem_SID_number is:'{0}'".format(json.dumps(_pubChem_SID_number,indent=4)));
        if(response__dict is not None and "InformationList" in response__dict and "Information" in response__dict["InformationList"] and len(response__dict["InformationList"]["Information"])):
            if("SID" in response__dict["InformationList"]["Information"][0]):
                pass;
                _pubChem_SID_number2=response__dict["InformationList"]["Information"][0]["SID"];
                if("Title" in response__dict["InformationList"]["Information"][0]):
                    pass;
                    cmpdname=response__dict["InformationList"]["Information"][0]["Title"];
                    
                    _pubChem_SID_info__dict[_pubChem_SID_number2]={
                        "record_ordinal_position":record_ordinal_position
                        ,"pubChem_SID":_pubChem_SID
                        #,"SID_id":SID_id
                        ,"SID_id":_pubChem_SID_number2
                        ,"cmpdname":cmpdname
                    };
        
    _SID_id__list=list(set(_SID_id__list));
    _SID_id__list.sort(reverse=False)
    
    LOGGER.info(">>>>>>>>>>>>>>>>>>>>>>>>>>>_pubChem_SID__list is:'{0}'".format(json.dumps(_pubChem_SID__list,indent=4)));
    LOGGER.info(">>>>>>>>>>>>>>>>>>>>>>>>>>>_SID_id__list is:'%s'"%(json.dumps(_SID_id__list,indent=4)));
    LOGGER.info(">>>>>>>>>>>>>>>>>>>>>>>>>>>_pubChem_SID_info__dict is:'%s'"%(json.dumps(_pubChem_SID_info__dict,indent=4)));
    return (_pubChem_SID_info__dict, _SID_id__list);

def obtain_pubChem_CID_description(
    pubChem_CID
):
    pass;
    #import urllib.parse;import urllib.request;import json;
    #url = "https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/cid/472181/description/JSON";params = {};
    url = "https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/cid/{0}/description/JSON".format(pubChem_CID);params = {};
    LOGGER.info("url is:'{0}', params are:'{1}'".format(url,json.dumps(params,indent=4,sort_keys=False)));
    data = urllib.parse.urlencode(params);data = data.encode("utf-8");req = urllib.request.Request(url, data);
    with urllib.request.urlopen(req) as f:
       response = f.read();
...
def obtain_pubChem_CID_info(
    ...
    for i, _pubChem_CID in enumerate(_pubChem_CID__list):
        record_ordinal_position=i;
        _pubChem_CID_number=_pubChem_CID.split("CID:")[-1];
        response__dict=obtain_pubChem_CID_description(
            _pubChem_CID_number
        );
        time.sleep(0.2);
        LOGGER.info(">>>>>>>>>>>>>>>>>>>>>>>>>>>_pubChem_CID_number is:'{0}'".format(json.dumps(_pubChem_CID_number,indent=4)));
        if("InformationList" in response__dict and "Information" in response__dict["InformationList"] and len(response__dict["InformationList"]["Information"])):
            if("CID" in response__dict["InformationList"]["Information"][0]):
                pass;
                _pubChem_CID_number2=response__dict["InformationList"]["Information"][0]["CID"];
                if("Title" in response__dict["InformationList"]["Information"][0]):
                    pass;
                    cmpdname=response__dict["InformationList"]["Information"][0]["Title"];
                    
                    _pubChem_CID_info__dict[_pubChem_CID_number2]={
                        "record_ordinal_position":record_ordinal_position
                        ,"pubChem_CID":_pubChem_CID
                        #,"CID_id":CID_id
                        ,"CID_id":_pubChem_CID_number2
                        ,"cmpdname":cmpdname
                    };
...
################obtain_gene_symbol__2__ko_id() (start) 2020-04-17 1310hrs#####################
def obtain_gene_symbol__2__ko_id(
    gene_symbol__list
):
    pass;
    _gene_symbol__2__ko_id__dict={};
    _ko_id__list=[];
    #import urllib.parse;import urllib.request;import json;
    #url = "https://www.uniprot.org/uploadlists/";params = {"from": "ID","to": "KO_ID","format": "tab","query": "O95954 O43766 O43505 Q9Y259 Q9UHN1"};
    url = "https://www.uniprot.org/uploadlists/";params = {"from": "ID","to": "KO_ID","format": "tab","query": " ".join(gene_symbol__list)};
    LOGGER.info("url is:'{0}'".format(url));
    data = urllib.parse.urlencode(params);data = data.encode("utf-8");req = urllib.request.Request(url, data);
    with urllib.request.urlopen(req) as f:
       response = f.read();
    #print(response.decode("utf-8"));
    response__tsv=response.decode("utf-8");
    str_io=io.StringIO(response__tsv);
    csv_reader=csv.reader(str_io,delimiter="\t",quotechar='"',quoting=csv.QUOTE_MINIMAL);
    row_cnt=0;
    for row in csv_reader:
        ...
        if(row_cnt>1):#ignore the header
            pass;
            gene_symbol=row[0];
            ko_id=row[1];
            if(gene_symbol not in _gene_symbol__2__ko_id__dict):
                _gene_symbol__2__ko_id__dict[gene_symbol]={
                    "record_ordinal_position__list":[]
                    ,"gene_symbol":gene_symbol
                    ,"ko_id__list":[]
                };
            _gene_symbol__2__ko_id__dict[gene_symbol]["record_ordinal_position__list"].append(row_cnt-1);
            _gene_symbol__2__ko_id__dict[gene_symbol]["ko_id__list"].append(ko_id);
            if(ko_id not in _ko_id__list):
    ...
    LOGGER.info(">>>>>>>>>>>>>>>>>>>>>>>>>>>_gene_symbol__2__ko_id__dict is:'%s'"%(json.dumps(_gene_symbol__2__ko_id__dict,indent=4)));
    return (_gene_symbol__2__ko_id__dict, _ko_id__list);
################obtain_gene_symbol__2__ko_id() (end) 2020-04-17 1320hrs#####################


################obtain_gene_info() (start) 2020-04-17 1736hrs###############################
"""
1)first search for the gene_id of the given gene symbol
-Use "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=gene&term=AASS&field=symbol&retmode=json&sort=relevance&retmax=10"
gene_id__list=response__dict["esearchresult"]["idlist"]

2)from the returned results pick the gene_id and compose a query using these gene_ids to obtain the gene_info
-Use "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esummary.fcgi?db=gene&id=10157&retmode=json&sort=relevance&retmax=10"
response__dict[""][""][""][""][""][""][""][""]
response__dict["result"][gene_id]["organism"]["taxid"]==9606
""";
def obtain_gene_info(
    ...
    _gene_info__dict={};
    _gene_id__list=[];
    record_ordinal_position=0;
    gene_symbol_query__cnt=0;
    for j, gene_symbol in enumerate(_gene_symbol__list):
        gene_symbol_query__cnt+=1;
        ...
        if(response__dict is not None and "esearchresult" in response__dict and "idlist" in response__dict["esearchresult"]):
            ...
            if("result" in response__dict and "uids" in response__dict["result"]):
                for i, gene_id in enumerate(response__dict["result"]["uids"]):
                    #
                    gene_name=response__dict["result"][gene_id]["name"];
                    taxon_id=response__dict["result"][gene_id]["organism"]["taxid"];
                    LOGGER.info("---------------------gene_symbol_query__cnt is:{0}, record_ordinal_position is:{1}, gene_symbol is'{2}', gene_name is:'{3}', gene_id is:{4}, taxon_id is:{5}".format(gene_symbol_query__cnt,record_ordinal_position,gene_symbol,gene_name,gene_id,taxon_id));
                    
                    if("organism" in response__dict["result"][gene_id]
                        and "taxid" in response__dict["result"][gene_id]["organism"]
                        and response__dict["result"][gene_id]["organism"]["taxid"]==9606
                    ):
                        record_ordinal_position+=1;
                        #gene_symbol_ordinal_position=j;
                        LOGGER.info("--------Found-------------gene_symbol_query__cnt is:{0}, record_ordinal_position is:{1}, gene_symbol is'{2}', gene_name is:'{3}', gene_id is:{4}, taxon_id is:{5}".format(gene_symbol_query__cnt,record_ordinal_position,gene_symbol,gene_name,gene_id,taxon_id));
    ...
    LOGGER.info(">>>>>>>>>>>>>>>>>>>>>>>>>>>_gene_id__list is:'%s'"%(json.dumps(_gene_id__list,indent=4,sort_keys=False)));
    LOGGER.info(">>>>>>>>>>>>>>>>>>>>>>>>>>>_gene_info__dict is:'%s'"%(json.dumps(_gene_info__dict,indent=4,sort_keys=False)));
    return (_gene_info__dict, _gene_id__list);
################obtain_gene_info() (end) 2020-04-17 1838hrs###############################
...
################obtain_gene_id__2__gene_symbol() 2020-04-17 1344hrs (start)#####################
def obtain_gene_id__2__gene_symbol(
    _gene_id__list
):
    pass;
    _gene_id__2__gene_symbol__dict={};
    _gene_symbol__list=[];
    
    #import urllib.parse;import urllib.request;import json;
    #url = "https://www.uniprot.org/uploadlists/";params = {"from": "P_ENTREZGENEID","to": "ID","format": "tab","query": "10841 10916 11019 11041 1120"};
    #url = "https://www.uniprot.org/uploadlists/";params = {"from": "P_ENTREZGENEID","to": "ID","format": "tab","query": " ".join(gene_id__list)};
    url = "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esummary.fcgi";params = {"db": "gene","retmode":"json","sort":"relevance","id": ",".join(_gene_id__list)};
    LOGGER.info("url is:'{0}'".format(url));
    data = urllib.parse.urlencode(params);data = data.encode("utf-8");req = urllib.request.Request(url, data);
    with urllib.request.urlopen(req) as f:
       response = f.read();
    ...
    if("result" in response__dict and "uids" in response__dict["result"]):
        for i, gene_id in enumerate(response__dict["result"]["uids"]):
            ...
            if(gene_id in _gene_id__list):
                ...
                if(gene_id not in _gene_id__2__gene_symbol__dict):
                        _gene_id__2__gene_symbol__dict[gene_id]={
                        "record_ordinal_position":gene_id_ordinal_position
                        ,"gene_id":gene_id
                        ,"gene_symbol__list":[]
                    };
                gene_symbol=response__dict["result"][gene_id]["name"];
                _gene_id__2__gene_symbol__dict[gene_id]["gene_symbol__list"].append(gene_symbol);
                _gene_symbol__list.append(gene_symbol);
    _gene_id__list=list(set(_gene_id__list));
    _gene_id__list.sort(reverse=False)
    LOGGER.info(">>>>>>>>>>>>>>>>>>>>>>>>>>>_gene_symbol__list is:'%s'"%(json.dumps(_gene_symbol__list,indent=4)));
    LOGGER.info(">>>>>>>>>>>>>>>>>>>>>>>>>>>_gene_id__2__gene_symbol__dict is:'%s'"%(json.dumps(_gene_id__2__gene_symbol__dict,indent=4)));
    return (_gene_id__2__gene_symbol__dict, _gene_symbol__list);
################obtain_gene_id__2__gene_symbol() 2020-04-17 1344hrs (end)#####################


################obtain_gene_id__2__uniprotkb_id() 2020-04-19 1724hrs (start)#####################
def obtain_gene_id__2__uniprotkb_id(
    _gene_id__list
):
    pass;
    _gene_id__2__uniprotkb_id__results__dict={};
    ...
    #import urllib.parse;import urllib.request;import json;
    #url = "https://www.uniprot.org/uploadlists/";params = {"from": "P_ENTREZGENEID","to": "ID","format": "tab","query": "10841 10916 11019 11041 1120"};
    url = "https://www.uniprot.org/uploadlists/";params = {"from": "P_ENTREZGENEID","to": "ID","format": "tab","query": " ".join(_gene_id__list)};
    LOGGER.info("url is:'{0}'".format(url));
    data = urllib.parse.urlencode(params);data = data.encode("utf-8");req = urllib.request.Request(url, data);
    with urllib.request.urlopen(req) as f:
       response = f.read();
    ...
    for row in csv_reader:
        ...
        if(row_cnt>1):#ignore the header
            ...
            gene_id=row[0];
            uniprotkb_id=row[1];
            if(gene_id not in _gene_id__2__uniprotkb_id__dict):
                _gene_id__2__uniprotkb_id__dict[gene_id]={
                    "record_ordinal_position__list":[]
                    ,"gene_id":gene_id
                    ,"uniprotkb_id__list":[]
                };
            _gene_id__2__uniprotkb_id__dict[gene_id]["record_ordinal_position__list"].append(row_cnt-1);
            _gene_id__2__uniprotkb_id__dict[gene_id]["uniprotkb_id__list"].append(uniprotkb_id);
            if(gene_id not in _gene_id__list):
                _gene_id__list.append(gene_id);
            if(uniprotkb_id not in _uniprotkb_id__list):
                _uniprotkb_id__list.append(uniprotkb_id);
    str_io=None;
    _gene_id__list2=list(set(_gene_id__list2));
    _gene_id__list2.sort(reverse=False)
    LOGGER.info(">>>>>>>>>>>>>>>>>>>>>>>>>>>_gene_id__list2 is:'%s'"%(json.dumps(_gene_id__list2,indent=4)));
    LOGGER.info(">>>>>>>>>>>>>>>>>>>>>>>>>>>_gene_id__2__uniprotkb_id__dict is:'%s'"%(json.dumps(_gene_id__2__uniprotkb_id__dict,indent=4)));
    _gene_id__2__uniprotkb_id__results__dict={
        "gene_id__2__uniprotkb_id__dict":_gene_id__2__uniprotkb_id__dict
        ,"gene_id__list":_gene_id__list2
        ,"uniprotkb_id__list":_uniprotkb_id__list
    };
    return _gene_id__2__uniprotkb_id__results__dict;
################obtain_gene_id__2__uniprotkb_id() 2020-04-19 1756hrs (end)#####################
...
def package_gene_id__2__uniprotkb_id(
    ...
    LOGGER.info(">>>>>>>>>>>>>>>>>>>>>>>>>>>_gene_symbol__list is:'%s'"%(json.dumps(_gene_symbol__list,indent=4)));
    LOGGER.info(">>>>>>>>>>>>>>>>>>>>>>>>>>>_gene_id__2__uniprotkb_id__dict is:'%s'"%(json.dumps(_gene_id__2__uniprotkb_id__dict,indent=4)));
    return (_gene_id__2__uniprotkb_id__dict, _gene_symbol__list);





################obtain_uniprotkb_id__2__ko_id() 2020-04-19 2200hrs (start)#####################
def obtain_uniprotkb_id__2__ko_id(
    _uniprotkb_id__list
):
    pass;
    _uniprotkb_id__2__ko_id__results__dict={};
    _uniprotkb_id__2__ko_id__dict={};
    _uniprotkb_id__list2=[];
    _ko_id__list=[];
    
    #import urllib.parse;import urllib.request;import json;
    #url = "https://www.uniprot.org/uploadlists/";params = {"from": "ID","to": "KO_ID","format": "tab","query": "O95954 O43766 O43505 Q9Y259 Q9UHN1"};
    url = "https://www.uniprot.org/uploadlists/";params = {"from": "ID","to": "KO_ID","format": "tab","query": " ".join(_uniprotkb_id__list)};
    LOGGER.info("url is:'{0}'".format(url));
    data = urllib.parse.urlencode(params);data = data.encode("utf-8");req = urllib.request.Request(url, data);
    with urllib.request.urlopen(req) as f:
       response = f.read();
    ...
    for row in csv_reader:
        ...
        if(row_cnt>1):#ignore the header
            ...
            uniprotkb_id=row[0];
            ko_id=row[1];
            if(uniprotkb_id not in _uniprotkb_id__2__ko_id__dict):
                _uniprotkb_id__2__ko_id__dict[uniprotkb_id]={
                    "record_ordinal_position__list":[]
                    ,"uniprotkb_id":uniprotkb_id
                    ,"ko_id__list":[]
                };
            _uniprotkb_id__2__ko_id__dict[uniprotkb_id]["record_ordinal_position__list"].append(row_cnt-1);
            _uniprotkb_id__2__ko_id__dict[uniprotkb_id]["ko_id__list"].append(ko_id);
            if(uniprotkb_id not in _uniprotkb_id__list):
                _uniprotkb_id__list.append(uniprotkb_id);
            if(ko_id not in _ko_id__list):
                _ko_id__list.append(ko_id);
    str_io=None;
    _uniprotkb_id__list2=list(set(_uniprotkb_id__list2));
    _uniprotkb_id__list2.sort(reverse=False)
    LOGGER.info(">>>>>>>>>>>>>>>>>>>>>>>>>>>_uniprotkb_id__list2 is:'%s'"%(json.dumps(_uniprotkb_id__list2,indent=4)));
    LOGGER.info(">>>>>>>>>>>>>>>>>>>>>>>>>>>_uniprotkb_id__2__ko_id__dict is:'%s'"%(json.dumps(_uniprotkb_id__2__ko_id__dict,indent=4)));
    _uniprotkb_id__2__ko_id__results__dict={
        "uniprotkb_id__2__ko_id__dict":_uniprotkb_id__2__ko_id__dict
        ,"uniprotkb_id__list":_uniprotkb_id__list2
        ,"ko_id__list":_ko_id__list
    };
    return _uniprotkb_id__2__ko_id__results__dict;
################obtain_uniprotkb_id__2__ko_id() 2020-04-19 2218hrs (end)#####################
...
def package_gene_symbol__2__ko_id(
    _gene_symbol__2__ko_id_tsv_file_name
    ,_gene_symbol__list
):
    pass;
    ...
    src_dataset_csv_fh=None;
    src_dataset_csv_reader=None;
    src_dataset_csv_fh=open(_gene_symbol__2__ko_id_tsv_file_name,"r");
    src_dataset_csv_reader=csv.reader(
        src_dataset_csv_fh
        ,delimiter="\t"
        ,quotechar='"'
    ...
    for row in src_dataset_csv_reader:
        ...
        if(row_cnt>1):
            ...
            if(gene_symbol in _gene_symbol__list):
                gene_id_ordinal_position=_gene_symbol__list.index(gene_symbol);
                _gene_symbol__2__ko_id__dict[gene_symbol]={
                    "record_ordinal_position":record_ordinal_position
                    ,"gene_symbol":gene_symbol
                    ,"ko_id":ko_id
...
def package_uniprotkb_id__2__ec_number(
    ...
    for row in src_dataset_csv_reader:
        ...
        if(row_cnt>1):
            ...
            if(gene_symbol in _gene_symbol__list):
                gene_id_ordinal_position=_gene_symbol__list.index(gene_symbol);
                _uniprotkb_id__2__ec_number__dict[gene_symbol]={
                    "record_ordinal_position":record_ordinal_position
                    ,"gene_symbol":gene_symbol
                    ,"ec_number":ec_number
                };
...
def cascade_fields_values(
    ...
    dest_dataset_csv_file_name=os.path.join(_dest_dir_file_name,"%s.clinical_logs_for_rdf_v2.csv"%(os.path.splitext(os.path.basename(_src_clinical_log_dataset_csv_file_name))[0]));
        ...
        if(1==1):
            ...
            if(row_cnt<=2):
                ...
                if(len(row_id)>0 and (1==2 or row_id__selection__list==None or len(row_id__selection__list)==0 or int(row_id)in row_id__selection__list)):
                    ...
                    ####here we include the list of uniprotkb_ids, we pair each gene_id to zero or one uniprotkb_id (start) 2020-04-17 1230hrs###############
                    uniprotkb_id__csv=None;
                    uniprotkb_id__list=[];
                    if("gene_id__2__uniprotkb_id__dict" in _gene_id__2__uniprotkb_id__results__dict):
                        for gene_id in gene_id__list:
                            if(gene_id in _gene_id__2__uniprotkb_id__results__dict["gene_id__2__uniprotkb_id__dict"]):
                                uniprotkb_id__asv="+".join(_gene_id__2__uniprotkb_id__results__dict["gene_id__2__uniprotkb_id__dict"][gene_id]["uniprotkb_id__list"]);
                                uniprotkb_id__list.append(uniprotkb_id__asv);
                            else:
                                uniprotkb_id__list.append("");
                    uniprotkb_id__csv=list_to_csv(uniprotkb_id__list);
                    ####here we include the list of uniprotkb_ids, we pair each gene_id to zero or one uniprotkb_id (end) 2020-04-17 1231hrs###############
                    """
                    ####here we include the list of gene_symbols, we pair each gene_id to zero or one gene_symbol (start) 2019-10-29 1510hrs###############
                    gene_symbol__csv=None;
                    gene_symbol__list=[];
                    for gene_id in gene_id__list:
                        if(gene_id in _gene_id__2__uniprotkb_id__dict):
                            gene_symbol=_gene_id__2__uniprotkb_id__dict[gene_id]["gene_symbol"];
                            gene_symbol__list.append(gene_symbol);
                        else:
                            gene_symbol__list.append("");
                    gene_symbol__csv=list_to_csv(gene_symbol__list);
                    ####here we include the list of gene_symbols, we pair each gene_id to zero or one gene_symbol (end) 2019-10-29 1517hrs###############
                    """;
                    
                    ####here we include the list of ko_ids, we pair each gene_id to zero or one ko_id (start) 2020-04-19 2248hrs###############
                    ko_id__csv=None;
                    ko_id__list=[];
                    if("uniprotkb_id__2__ko_id__dict" in _uniprotkb_id__2__ko_id__results__dict):
                        for uniprotkb_id in uniprotkb_id__list:
                            if(uniprotkb_id in _uniprotkb_id__2__ko_id__results__dict["uniprotkb_id__2__ko_id__dict"]):
                                ko_id__asv="+".join(_uniprotkb_id__2__ko_id__results__dict["uniprotkb_id__2__ko_id__dict"][uniprotkb_id]["ko_id__list"]);
                                ko_id__list.append(ko_id__asv);
                            else:
                                ko_id__list.append("");
                    ko_id__csv=list_to_csv(ko_id__list);
                    ####here we include the list of uniprotkb_ids, we pair each gene_id to zero or one uniprotkb_id (end) 2020-04-19 2248hrs###############
                    
                    """
                    ####here we include the list of gene_symbols, we pair each gene_symbol to zero or one ko_id (start) 2019-10-29 1650rs###############
                    ko_id__csv=None;
                    ko_id__list=[];
                    for gene_symbol in gene_symbol__list:
                        if(gene_symbol in _gene_symbol__2__ko_id__dict):
                            ko_id__asv="+".join(_gene_symbol__2__ko_id__dict[gene_symbol]["ko_id__list"]);
                            ko_id__list.append(ko_id__asv);
                        else:
                            ko_id__list.append("");
                    ko_id__csv=list_to_csv(ko_id__list);
                    ####here we include the list of gene_symbols, we pair each gene_id to zero or one gene_symbol (end) 2019-10-29 1652hrs###############
                    """;
                    ####here we include the list of gene_symbols, we pair each gene_symbol to zero or one ec_number (start) 2019-10-30 1104hrs###############
                    ec_number__csv2=None;
                    ec_number__list2=[];
                    for gene_symbol in gene_symbol__list:
                        if(gene_symbol in _uniprotkb_id__2__ec_number__dict):
                            ec_number=_uniprotkb_id__2__ec_number__dict[gene_symbol]["ec_number"];
                            ec_number__list2.append(ec_number);
                        else:
                            ec_number__list2.append("");
                    ec_number__csv2=list_to_csv(ec_number__list2);
                    ####here we include the list of gene_symbols, we pair each gene_id to zero or one gene_symbol (end) 2019-10-30 1104hrs###############
...
14    enzyme    has_id    http://bio2vec.net/has_id    expasy or uniprotkb_id    expasy or uniprotkb_id
...
if __name__ == '__main__':
    ...
    gene_symbol__2__ko_id_tsv_file_name=None;
    uniprotkb_id__2__ec_number_tsv_file_name=None;
    PubChem_CID_csv_file_name=None;
    ...
    try:
        ...
        parser.add_argument("-k","--gene_symbol__2__ko_id_tsv_file_name",action="append",type=str,dest="op__gene_symbol__2__ko_id_tsv_file_name",help="The full path to the UniProtKB_id to KO_id mapping TSV text file.")
        ...
        if len(sys.argv)<4:
            parser.error("""
                ...
                -k, --gene_symbol__2__ko_id_tsv_file_name
        ...
        if(options.op__gene_symbol__2__ko_id_tsv_file_name):
            gene_symbol__2__ko_id_tsv_file_name=options.op__gene_symbol__2__ko_id_tsv_file_name[0];
    ...
    LOGGER.info(" '%s', -------------- gene_symbol__2__ko_id_tsv_file_name is:'%s'"%(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f'),gene_symbol__2__ko_id_tsv_file_name));
    ...
    try:
        for i in range(count_of_workers):
            ...
            worker_id2='%s__%s__%d'%('BORG_DDIEM__prepare_clinical_logs_for_rdf_v2',worker_id,i);
            LOGGER.info(">>>>>>>>>>worker_tag:'%s'"%(worker_id2));
            
            worker_number=i;
            w=BORG_DDIEM__prepare_clinical_logs_for_rdf_v2(
                ...
                ,gene_symbol__2__ko_id_tsv_file_name
                ,uniprotkb_id__2__ec_number_tsv_file_name
                ,PubChem_CID_csv_file_name
            );
            """
            t=threading.Thread(
                target=run_BORG_DDIEM__prepare_clinical_logs_for_rdf_v2
                ,args=(
                    d
                    ,worker_id2
                )
